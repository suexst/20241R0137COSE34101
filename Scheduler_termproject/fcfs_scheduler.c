...
void fcfs_add_process(Scheduler* scheduler, Process* process) {
    pthread_mutex_lock(&scheduler->lock);
    enqueue(scheduler->ready_queue, process);
    pthread_mutex_unlock(&scheduler->lock);
}

Process* fcfs_get_next_process(Scheduler* scheduler) {
    pthread_mutex_lock(&scheduler->lock);
...
void fcfs_schedule(Scheduler* scheduler, int num_thread) {
    Process* process;
    pthread_t tid;
    int count = 0;
    ...
    while(count < num_thread) {
        if ((process = fcfs_get_next_process(scheduler)) != NULL) {
...
Scheduler* create_fcfs_scheduler(int num_thread) {
    ...
    scheduler->add_process = fcfs_add_process;
    scheduler->get_next_process = fcfs_get_next_process;
