...
void sjf_add_process(Scheduler* scheduler, Process* process) {
...
Process* sjf_get_next_process(Scheduler* scheduler) {
    pthread_mutex_lock(&scheduler->lock);
...
void sjf_schedule(Scheduler* scheduler, int num_thread) {
    Process* process;
    pthread_t tid;
    int count = 0;
    ...
    while(count < num_thread) {
        if ((process = sjf_get_next_process(scheduler)) != NULL) {
...
Scheduler* create_sjf_scheduler(int num_thread) {
    ...
    scheduler->add_process = sjf_add_process;
    scheduler->get_next_process = sjf_get_next_process;
